...
class I2CInitiator(Module):
    ...
    def __init__(self, pads, period_cyc, clk_stretch=True):
        ...
        self.sync += [
            If((timer == 0) | ~self.busy, timer.eq(period_cyc // 4)).Elif(
                (not clk_stretch) | (bus.scl_o == bus.scl_i), timer.eq(timer - 1)
            )
        ]
        ...
        def scl_l(state, next_state, *exprs):
            self.fsm.act(
                state, If(stb, NextValue(bus.scl_o, 0), NextState(next_state), *exprs)
            )

        def scl_h(state, next_state, *exprs):
            self.fsm.act(
                state,
                If(stb, NextValue(bus.scl_o, 1)).Elif(
                    bus.scl_o == 1,
                    If(
                        (not clk_stretch) | (bus.scl_i == 1),
        ...
        def stb_x(state, next_state, *exprs):
            self.fsm.act(state, If(stb, NextState(next_state), *exprs))
        ...
        self.fsm.act(
            ...
            If(
                self.start,
                If(bus.scl_i & bus.sda_i, NextState("START-SDA-L"))
                .Elif(~bus.scl_i, NextState("START-SCL-H"))
                .Elif(bus.scl_i, NextState("START-SCL-L")),
            )
            .Elif(
                self.stop,
                If(bus.scl_i & ~bus.sda_o, NextState("STOP-SDA-H"))
                .Elif(~bus.scl_i, NextState("STOP-SCL-H"))
                .Elif(bus.scl_i, NextState("STOP-SCL-L")),
            )
            .Elif(
                self.write,
            ...
            .Elif(self.read, NextValue(r_ack, self.ack_i), NextState("READ-DATA-SCL-L"))
            .Else(NextValue(self.busy, 0)),
        )
        # start
        scl_l("START-SCL-L", "START-SDA-H")
        stb_x("START-SDA-H", "START-SCL-H", NextValue(bus.sda_o, 1))
        scl_h("START-SCL-H", "START-SDA-L")
        stb_x("START-SDA-L", "IDLE", NextValue(bus.sda_o, 0))
        # stop
        scl_l("STOP-SCL-L", "STOP-SDA-L")
        stb_x("STOP-SDA-L", "STOP-SCL-H", NextValue(bus.sda_o, 0))
        scl_h("STOP-SCL-H", "STOP-SDA-H")
        stb_x("STOP-SDA-H", "IDLE", NextValue(bus.sda_o, 1))
        # write data
        scl_l("WRITE-DATA-SCL-L", "WRITE-DATA-SDA-X")
        stb_x("WRITE-DATA-SDA-X", "WRITE-DATA-SCL-H", NextValue(bus.sda_o, w_shreg[7]))
        scl_h(
            "WRITE-DATA-SCL-H",
            "WRITE-DATA-SDA-N",
        ...
        stb_x(
            ...
            If(bitno == 7, NextState("WRITE-ACK-SCL-L")),
        )
        # write ack
        scl_l("WRITE-ACK-SCL-L", "WRITE-ACK-SDA-H")
        stb_x("WRITE-ACK-SDA-H", "WRITE-ACK-SCL-H", NextValue(bus.sda_o, 1))
        scl_h("WRITE-ACK-SCL-H", "WRITE-ACK-SDA-N", NextValue(self.ack_o, ~bus.sda_i))
        stb_x("WRITE-ACK-SDA-N", "IDLE")
        # read data
        scl_l("READ-DATA-SCL-L", "READ-DATA-SDA-H")
        stb_x("READ-DATA-SDA-H", "READ-DATA-SCL-H", NextValue(bus.sda_o, 1))
        scl_h(
            "READ-DATA-SCL-H",
            "READ-DATA-SDA-N",
        ...
        stb_x(
            ...
            "READ-DATA-SCL-L",
            NextValue(bitno, bitno + 1),
            If(bitno == 7, NextState("READ-ACK-SCL-L")),
        )
        # read ack
        ...
        scl_h("READ-ACK-SCL-H", "READ-ACK-SDA-N", NextValue(self.data_o, r_shreg))
    ...
    def __init__(self, pads):
        ...
        self.fsm.act(
            ...
            If(
                ...
                NextState("IDLE"),
            ).Elif(bus.setup, NextValue(bitno, 0), NextState("ADDR-SHIFT")),
        )
        self.fsm.act(
            "ADDR-SHIFT",
            If(bus.stop, NextState("IDLE"))
            .Elif(bus.start, NextState("START"))
            .Elif(
                bus.sample,
        ...
        self.fsm.act(
            "ADDR-ACK",
            If(bus.stop, self.stop.eq(1), NextState("IDLE"))
            .Elif(bus.start, self.restart.eq(1), NextState("START"))
            .Elif(
                bus.setup,
                If(~shreg_i[0], NextValue(bus.sda_o, 1), NextState("WRITE-SHIFT")),
            )
            .Elif(
                bus.sample,
                If(
        ...
        self.fsm.act(
            ...
            .Elif(
                bus.setup,
                NextValue(bitno, bitno + 1),
                If(bitno == 7, NextValue(self.data_i, shreg_i), NextState("WRITE-ACK")),
            ),
        )
        ...
        self.fsm.act(
            ...
            If(bus.stop, self.stop.eq(1), NextState("IDLE"))
            .Elif(bus.start, self.restart.eq(1), NextState("START"))
            .Elif(bus.setup, NextValue(bus.sda_o, 1), NextState("WRITE-SHIFT"))
            .Elif(
                ~bus.scl_i,
                NextValue(bus.scl_o, ~self.busy),
                If(self.ack_o, NextValue(bus.sda_o, 0)),
            ),
        )
        self.comb += self.read.eq(self.fsm.before_entering("READ-STRETCH"))
        self.fsm.act(
            "READ-STRETCH",
            If(self.busy, NextValue(shreg_o, self.data_o)),
            If(bus.stop, self.stop.eq(1), NextState("IDLE"))
            .Elif(bus.start, NextState("START"))
            .Elif(self.busy, If(~bus.scl_i, NextValue(bus.scl_o, 0)))
            .Else(
                If(~bus.scl_i, NextValue(bus.sda_o, shreg_o[7])),
        ...
        self.fsm.act(
            ...
            .Elif(
                ...
                If(
                    ...
                    NextValue(shreg_o, self.data_o),
                    NextState("READ-STRETCH"),
                ).Else(self.stop.eq(1), NextState("IDLE")),
            ),
        )
